...
if __name__ == "__main__":
    for date_idx, date in enumerate(dates):
        for wl_idx, filt_name in enumerate(names):
            ...
            for param_idx in range(4):
                lo_67, hi_67 = arviz.hdi(data["samples"][:, param_idx + 4], hdi_prob=0.67)
                lo_95, hi_95 = arviz.hdi(data["samples"][:, param_idx + 4], hdi_prob=0.95)

                x_val = wavelengths[wl_idx]
                axes[param_idx, date_idx].scatter(
                    [x_val], [meds[param_idx + 4]], ec=cycle[wl_idx], ms=10, fc="w", zorder=999
                )
    ...
    fig.savefig(
        paths.figures / "HD169142_vampires_outer_ring_posteriors.pdf",
        bbox_inches="tight",
        dpi=300,
    )
