...
if __name__ == "__main__":
    ...
    circ = patches.Ellipse(
        (0, 0),
        width=b0_width * np.cos(np.deg2rad(35.2 - 12.45)),
        height=b0_width,
        angle=31.55 - 5.88,
        **alma_patch_kwargs,
    )
    ...
    ax.text(
        r_d1 * np.cos(np.deg2rad(ang)),
        r_d1 * np.sin(np.deg2rad(ang)),
        "D1",
        va="center",
        **alma_text_kwargs,
    )
    ...
    ax.text(
        r_d2 * np.cos(np.deg2rad(ang)),
        r_d2 * np.sin(np.deg2rad(ang)),
        "D2",
        va="center",
        **alma_text_kwargs,
    )
    ...
    ax.text(
        r_d3 * np.cos(np.deg2rad(ang + 2)),
        r_d3 * np.sin(np.deg2rad(ang + 2)),
        "D3",
        va="center",
        **alma_text_kwargs,
    )
    ...
    ax.text(
        r_d4 * np.cos(np.deg2rad(ang - 2)),
        r_d4 * np.sin(np.deg2rad(ang - 2)),
        "D4",
        va="center",
        **alma_text_kwargs,
    )
    ...
    text_kwargs = dict(
        ...
        ha="center",
        fontweight="bold",
        bbox=dict(boxstyle="square", facecolor="w", edgecolor="C0"),
        zorder=20,
    )
    ...
    circ = patches.Circle((x, y), 3, ec="0.2", fill=False)
    ax.add_patch(circ)
    ax.scatter(x, y, marker="+", c="0.2", s=10, lw=0.5)
    ax.text(x + 5, y + 4, "b", fontsize=6, ha="center", va="center", c="0.2")
