...
if __name__ == "__main__":
    ...
    for col_idx in range(len(folders) - 1):
        ...
        for row_idx in range(col_idx, len(folders)):
            ...
            extrap_err = np.interp(
                common_lag, lags[inds], xcorr_err[inds], left=np.nan, right=np.nan
            )
    ...
    axes.format(
        xlabel="Motion (°/yr)", ylabel="Phase cross-correlation", yformatter="none", xlocator=20
    )

    ymin, ymax = axes[0].get_ylim()
    inner_kep = [-5.2 - 1, -5.2 + 1]
