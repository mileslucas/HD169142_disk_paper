...
for date in ("20230707",  "20240729"):
    rdi_path = (
        paths.data / date / "rdi_residuals" / f"{date}_HD169142_vampires_ipca_adi+rdi.fits"
    )
    rdi_cube, header = fits.getdata(rdi_path, header=True)
    fig = plot_rdi_mosaic(rdi_cube, iwa=iwas[date], smooth=False)

    fig.savefig(
        paths.figures / f"{date}_HD169142_ipca_adi+rdi_mosaic.pdf",
        dpi=300,
    )

for date in ("20230707",  "20240729"):
    rdi_path = (
        paths.data / date / "rdi_residuals" / f"{date}_HD169142_vampires_ipca_rdi.fits"
    )
    rdi_cube, header = fits.getdata(rdi_path, header=True)
    fig = plot_rdi_mosaic(rdi_cube, iwa=iwas[date], smooth=False)

    fig.savefig(
        paths.figures / f"{date}_HD169142_ipca_rdi_mosaic.pdf",
        dpi=300,
    )
