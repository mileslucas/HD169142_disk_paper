...
if __name__ == "__main__":
    ...
    for i, folder in enumerate(folders):
        ...
        rect = patches.Rectangle(
            [0.75, -0.8 - bar_width_height / 2], -bar_width_arc, bar_width_height, color="white"
        )
    ...
    axes.format(xlim=(0.9, -0.9), ylim=(-0.9, 0.9), xlocator="none", ylocator="none")
    axes[:, 0].format(ylocator=[-0.6, -0.3, 0, 0.3, 0.6], ylabel=r'$\Delta$DEC (")')
    axes[-1, :].format(xlocator=[-0.6, -0.3, 0, 0.3, 0.6], xlabel=r'$\Delta$RA (")')

    # axes[1].format(yspineloc="none")

    fig.savefig(paths.figures / "HD169142_Qphi_mosaic.pdf", bbox_inches="tight")
    ...
    fig.savefig(paths.figures / "HD169142_Qphi_ALMA_mosaic.pdf", bbox_inches="tight")
