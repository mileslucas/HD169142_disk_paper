...
def rdi_residuals(
    ...
    opt_smoothing=False,
    opt_smoothing_fn=None,
    opt_smoothing_kwargs=None,
):
    """
    ...
    if opt_smoothing:
        ...
        hcube_opt, hcube_ref_opt = (
            opt_smoothing_fn(hcube_sub, **opt_smoothing_kwargs),
            opt_smoothing_fn(hcube_ref, **opt_smoothing_kwargs),
        )
    ...
    if hcube.ndim == hcube_ref.ndim:
        if isNone(coeffs_in):
            coeffs = compute_rdi_coefficients(
                hcube_opt,
                hcube_ref_opt,
                optzones,
        ...
        if return_coeffs:
            return coeffs
        psf_model = reconstruct_psf_model(
            hcube_ref,
            coeffs,
            subzones,
            show_progress=show_progress,
    ...
    if not isNone(parangs):
        if pad_before_derot:
            hcube_res = pad_and_rotate_hypercube(
                hcube_res,
                -parangs,
                cent=cent,
                ncores=ncores,
...
def adi_residuals(
    ...
    if isNone(r_opt):
        if isNone(cent):
            cent = (np.array(hcube.shape[-2:][::-1]) - 1) / 2.0
        rmap = dist_to_pt(
            cent, nx=hcube.shape[-1], ny=hcube.shape[-2], dtype=hcube.dtype
        )
        r_opt = np.array([np.min(rmap[optzone]) for optzone in optzones])

    coeffs = compute_adi_coefficients(
        hcube_sub,
        parangs,
        optzones,
        r_opt,
...
def compute_rdi_coefficients(
    hcube, hcube_ref, optzones, show_progress=False, ref_mask=None
):
    if show_progress:
        try:
            from tqdm.auto import tqdm
    ...
    R_iterator = (
        tqdm(range(nR), desc="Coefficient calculation (regions)", leave=False)
        if show_progress
        else range(nR)
    )
    L_iterator = (
        tqdm(range(nL), desc="Coefficient calculation (wavelengths)", leave=False)
        if (show_progress and nL > 1)
        else range(nL)
    )
    T_iterator = (
        tqdm(range(nT), desc="Coefficient calculation (exposures)", leave=False)
        if show_progress
        else range(nT)
    )
    for Ri in R_iterator:  # Outermost loop over subsections; this iteration order ends up being more time efficient here:
        opt_i = optzones[
            Ri
        ]  # opt_i is the ny by nx boolean array indicating which pixels are in the region.
        tararrs = hcube[
            :, :, opt_i
        ].copy()  # This turns our masked nT*nL*ny*nx array into an nT*nL*nP array, where nP is the number of pixels in the optimization region.
        refarrs = hcube_ref[
            :, :, opt_i
        ].copy()  # This turns our masked nT_ref*nL*ny*nx array into an nT_ref*nL*nP array, where nP is the number of pixels in the optimization region.
        if not isNone(ref_mask):
            refarrs = refarrs[ref_mask[Ri]]
        optmats = (
            refarrs.transpose((1, 0, 2)) @ tararrs.transpose((1, 2, 0))
        )  # Using python 3.6+ magic, carries out matrix inversion of all wavelength channels at once, giving a matrix of shape (nL, nT_ref, nT)
        refmats = (
            refarrs.transpose((1, 0, 2)) @ refarrs.transpose((1, 2, 0))
        )  # Using python 3.6+ magic, carries out matrix inversion of all wavelength channels at once, giving a matrix of shape (nL, nT_ref, nT_ref)
        for Li in L_iterator:  # Second loop over wavelengths:
            ...
            lu, piv = linalg.lu_factor(
                refmat
            )  # Since we aren't excluding frames as in ADI/SDI, we just need to run this once per wavelength.
            for Ti in (
                T_iterator
            ):  # Final loop over par ang / time axis (individual science exposures):
                tararr = optmat[:, Ti]  # 1d vector of length equal to nT_ref
                if not isNone(ref_mask):
                    coeff_hcube[Li, Ri, Ti, ref_mask[Ri]] = linalg.lu_solve(
                        (lu, piv), tararr
                    )  # Gets coefficients and places them into the appropriate positions in the coefficient array
                else:
                    coeff_hcube[Li, Ri, Ti] = linalg.lu_solve(
                        (lu, piv), tararr
                    )  # Gets coefficients and places them into the appropriate positions in the coefficient array
    return coeff_hcube


def compute_adi_coefficients(
    hcube, parangs, optzones, r1, nfwhm, fwhm_array, show_progress=False
):
    if show_progress:
        try:
            from tqdm.auto import tqdm
    ...
    for Ri in R_iterator:  # Outermost loop over subsections; this iteration order ends up being more time efficient here:
        ...
        optmats = (
            optarrs.transpose((1, 0, 2)) @ optarrs.transpose((1, 2, 0))
        )  # Using python 3.6+ magic, carries out matrix inversion of all 22 wavelength channels at once, giving a matrix of shape (nL, nT_ref, nT)
        for Li in L_iterator:  # Second loop over wavelengths:
            min_disp = min_disps[
                Li
            ]  # The minimum displacement in pixels for this wavelength
            ...
            for Ti in T_iterator:  # Final loop over par ang / time axis (individual science exposure cubes):
                dtheta = (
                    parangs - parangs[Ti]
                )  # Difference between parang of each frame and the target frame
                disp = ang_displacement(
                    r1[Ri], dtheta
                )  # Corresponding pixel displacement at the inner edge of the optimization region.
                valid_dtheta = (
                    disp > min_disp
                )  # Mask of shape (nT,) indicating which frames are valid references. Choosing > rather than >= just ensures a non-trivial result in the case for min_disp = 0 (where >= would include the tar image)
                if np.sum(valid_dtheta) == 0.0:
                    continue  # Move on if no valid frames are available for this subsection.
                optmat_v1 = optmat[valid_dtheta]
                tararr = optmat_v1[
                    :, Ti
                ]  # 1d vector of length equal to the number of "valid" images in sequence
                refmat = optmat_v1[:, valid_dtheta]
                lu, piv = linalg.lu_factor(refmat)
                coeff_hcube[Li, Ri, Ti, valid_dtheta] = linalg.lu_solve(
                    (lu, piv), tararr
                )
    return coeff_hcube


def reconstruct_psf_model(
    hcube_ref, coeffs, subzones, show_progress=False, use_gpu=False, ncores=-2
):
    if use_gpu:
        hcube_psfmodel = reconstruct_psf_model_gpu(
            hcube_ref, coeffs, subzones, show_progress=show_progress
        )
    else:
        hcube_psfmodel = reconstruct_psf_model_cpu(
            hcube_ref, coeffs, subzones, show_progress=show_progress, ncores=ncores
        )
    return hcube_psfmodel


def reconstruct_psf_model_cpu(
    hcube_ref, coeffs, subzones, show_progress=False, ncores=-2
):
    if show_progress:
        try:
            from tqdm.auto import tqdm
    ...
    (
        ...
        ny,
        nx,
    ) = hcube_ref.shape  # Number of: images, wavelengths, y-pixels, and x-pixels
    nL, nR, nT, _ = coeffs.shape
    hcube_psfmodel = (
        np.zeros((nT, nL, ny, nx)) + np.nan
    )  # Array in which to place reconstructed model
    R_iterator = (
        tqdm(range(nR), desc="PSF model reconstruction (regions)", leave=False)
        if show_progress
        else range(nR)
    )  # Iterator for regions
    T_iterator = (
        tqdm(range(nT), desc="PSF model reconstruction (exposures)", leave=False)
        if show_progress
        else range(nT)
    )  #  Iterator for images
    for Ri in R_iterator:
        sub_i = subzones[
            Ri
        ]  # An ny by nx boolean mask indicating which pixels are being considered
        imvals = (
            hcube_ref[:, :, sub_i].T.copy()
        )  # Fetching the pixels in the subzone, dimensions of nT_ref x nL x npx
        nT_results = Parallel(n_jobs=ncores, prefer="threads")(
            delayed(np.sum)(coeffs[np.newaxis, :, Ri, Ti].copy() * imvals, -1)
            for Ti in T_iterator
        )
        for Ti in range(
            nT
        ):  # Iterate over target images, building the PSF model (in the subzone) for each.
            hcube_psfmodel[Ti, :, sub_i] = nT_results[
                Ti
            ]  # Multiply images by the coefficients, then sum along the nT axis to get the model values.
    return hcube_psfmodel
...
def reconstruct_psf_model_gpu(hcube_ref, coeffs, subzones, show_progress=False):
    ...
    (
        ...
        ny,
        nx,
    ) = cp_hcube_ref.shape  # Number of: images, wavelengths, y-pixels, and x-pixels
    nL, nR, nT, _ = cp_coeffs.shape

    hcube_psfmodel = (
        cp.zeros((nT, nL, ny, nx)) + np.nan
    )  # Array in which to place reconstructed model
    R_iterator = (
        tqdm(range(nR), desc="PSF model reconstruction (regions)", leave=False)
        if show_progress
    ...
    for Ri in R_iterator:
        ...
        imvals = cp_hcube_ref[:, :, sub_i].transpose(
            (1, 0, 2)
        )  # Fetching the pixels in the subzone, dimensions of nT_ref x nL x npx
        for Ti in (
            tqdm(T_iterator, desc="Target image", leave=False)
            if show_progress
            else T_iterator
        ):  # Iterate over target images, building the PSF model (in the subzone) for each.
            cvals = cp_coeffs[
            ...
            hcube_psfmodel[Ti, :, sub_i] = cp.sum(
                cvals * imvals, axis=1
            )  # Multiply images by the coefficients, then sum along the nT axis to get the model values.
    hcube_psfmodel_np = cp.asnumpy(hcube_psfmodel)  # Convert output back to numpy
    hcube_psfmodel, cp_hcube_ref, cp_coeffs, cp_subzones, cvals = free_gpu(
        hcube_psfmodel, cp_hcube_ref, cp_coeffs, cp_subzones, cvals
    )  # Explicitly clear VRAM for cupy arrays
...
def rdi_greeds(
    ...
    crop_shape = np.repeat(
        int(np.ceil(2 * (np.sqrt(2) * np.max(rmap[np.any(optzones, axis=0)])))) + 5, 2
    )
    cr_hcube, cr_cent = crop_data(hcube, crop_shape, cent)
    cr_hcube_ref, cr_optzones, cr_subzones = [
        crop_data(i, crop_shape, cent)[0] for i in [hcube_ref, optzones, subzones]
    ]
    ...
    for i in n_iterator:
        if i == 0:
            ...
            hcube_css = imcube_to_adi_hcube(
                css * (css > 0), parangs, cent=cr_cent, use_gpu=use_gpu, ncores=ncores
            )
        res_hcube = rdi_residuals(
            cr_hcube,
            cr_hcube_ref,
            cr_optzones,
            cr_subzones,
...
def adi_greeds(
    ...
    for i in n_iterator:
        if i == 0:
            ...
            hcube_css = imcube_to_adi_hcube(
                np.nan_to_num(css * (css > 0)),
                parangs,
                cent=cent,
                use_gpu=use_gpu,
                ncores=ncores,
            )
        res_hcube = adi_residuals(
            hcube,
            parangs,
            optzones,
            subzones,
...
def setup_display(width=95, fontsize=18):
    ...
    display(
        HTML("<style>.rendered_html { font-size: " + str(fontsize) + "px; }</style>")
    )
    return None
...
def rotate_image_cpu(
    im, angle, cent=None, new_cent=None, cval0=np.nan, prop_threshold=1e-6
):
    """
...
def rotate_image_gpu(im0, angle, cent=None, cval0=np.nan):
    ...
    if isNone(cent) or np.all(cent == geom_cent):
        if any_nans:
            rot_im = cp_ndimage.rotate(
                cp.where(nans, 0.0, im), angle, axes=(-2, -1), reshape=False, cval=cval0
            )
        else:
            rot_im = cp_ndimage.rotate(
                im, angle, axes=(-2, -1), reshape=False, cval=cval0
            )
        if any_zeros:
            rot_zeros = cp_ndimage.rotate(
                zeros.astype(float),
                angle,
                axes=(-2, -1),
                prefilter=False,
            ...
            rot_im = cp.where(rot_zeros > 0.0, 0.0, rot_im)
        if any_nans:
            rot_nans = cp_ndimage.rotate(
                nans.astype(float), angle, axes=(-2, -1), prefilter=False, reshape=False
            )
            rot_im = cp.where(rot_nans > 0.0, cp.nan, rot_im)
    else:
        if any_nans:
            rot_im = rotate_about_pos_gpu(
                cp.where(nans, 0.0, im), cent, angle, cval=cval0
            )
        else:
            rot_im = rotate_about_pos_gpu(im, cent, angle, cval=cval0)
        if any_zeros:
            rot_zeros = rotate_about_pos_gpu(
                zeros.astype(float), cent, angle, prefilter=False
            )
            rot_im = cp.where(rot_zeros > 0.0, 0.0, rot_im)
        if any_nans:
            rot_nans = rotate_about_pos_gpu(
                nans.astype(float), cent, angle, prefilter=False
            )
            rot_im = cp.where(rot_nans > 0.0, cp.nan, rot_im)
    return cp.asnumpy(rot_im)


def rotate_about_pos_gpu(
    im, pos, angle, cval=np.nan, order=3, mode="constant", prefilter=True
):
    ny, nx = im.shape[-2:]
    nd = cp.ndim(im)
    xg0, yg0 = cp.meshgrid(
        cp.arange(nx, dtype=cp.float64), cp.arange(ny, dtype=cp.float64)
    )
...
def rotate_hypercube(
    hcube, angles, cent=None, new_cent=None, ncores=-2, use_gpu=False, cval0=0.0
):
    """
    ...
    if use_gpu:
        rot_hcube = np.stack(
            [
                rotate_image_gpu(
                    imcube, angle, cval0=cval0, cent=cent, new_cent=new_cent
                )
...
def pad_and_rotate_hypercube(
    hcube, angles, cent=None, ncores=-2, use_gpu=False, cval0=np.nan
):
    """
    ...
    corner_coords = np.array(
        [[dxmax, dymax], [dxmax, dymin], [dxmin, dymin], [dxmin, dymax]]
    )
    uni_angs = np.unique(angles)
    derot_corner_coords = np.vstack(
        [
            np.array(xy_polar_ang_displacement(*corner_coords.T, -ang)).T
            for ang in uni_angs
        ]
    )
    dxmin_pad, dymin_pad = (
        np.ceil(np.abs(np.min(derot_corner_coords, axis=0) - np.array([dxmin, dymin])))
    ).astype(int)
    dxmax_pad, dymax_pad = (
        np.ceil(np.abs(np.max(derot_corner_coords, axis=0) - np.array([dxmax, dymax])))
    ).astype(int)
    hcube_pad = np.pad(
        hcube.copy(),
        [
            *[[0, 0] for i in range(hcube.ndim - 2)],
            [dymin_pad, dymax_pad],
            [dxmin_pad, dxmax_pad],
        ],
    ...
    hcube_pad_rot = rotate_hypercube(
        hcube_pad, angles, cent=cent_pad, ncores=ncores, use_gpu=use_gpu, cval0=cval0
    )
    return hcube_pad_rot, cent_pad


def rotate_about_pos(
    im, pos, angle, new_cent=None, cval=np.nan, order=3, mode="constant", prefilter=True
):
    ny, nx = im.shape[-2:]
    nd = np.ndim(im)
    xg0, yg0 = np.meshgrid(
...
def gpu_nanmedian(x, axis=None):
    ...
    if avail_mem < (x.nbytes * 6):
        ...
        for inds in split_inds:
            out[
                index_axis(inds, np.where(preserved_axes == split_axis)[0][0])
            ] = cp.nanmedian(x[index_axis(inds, split_axis)], axis=axis)
            free_gpu()
    else:
...
def quick_implot(
            ...
            if s.isdigit():
                ...
                if "*" in s:
                    ...
                    for si in s.split("*"):
                        if "%" in si:
                            svals.append(
                                np.nanpercentile(im, float(si.replace("%", "")))
                            )
                        else:
    ...
    if n_ims == 1:
        ...
        im = np.asarray(im).reshape(
            (np.product(imshape[0:-2]), imshape[-2], imshape[-1])
        )
...
def animate_quick_implot(
    ...
    **quick_implot_kwargs,
    ...
    if isNone(titles):
        titles = np.arange(len(im_cube)).astype(str)
    else:
        titles = np.asarray(titles).astype(str)
    ...
    anim = animation.FuncAnimation(
        fig, animate, frames=len(im_cube), interval=dur * 1000 / len(im_cube)
    )
    plt.close()
    return anim
...
def model_rescale_factor(A, B, sig=None, mask=None):
    ...
    if not isNone(sig):
        if np.shape(A) != np.shape(sig):
            raise ValueError(
                "A, B, and sig must be arrays of the same shape if sig is specified!"
            )
    else:
        sig = 1
    if isNone(mask):
...
def ang_displacement(r, dtheta):
    ...
    """
    return 2 * r * np.abs(np.sin(np.deg2rad(dtheta / 2.0)))
...
def sigma_clipped_axis_nanmean_general(
    x, n=2.0, axis=0, clip_mask=None, return_clip_mask=False, use_gpu=False, fast=False
):
    if use_gpu:
        res = sigma_clipped_axis_nanmean_gpu(
            x, n=n, axis=axis, clip_mask=clip_mask, return_clip_mask=return_clip_mask
        )
    elif fast:
        res = sigma_clipped_axis_nanmean_fast(
            x, n=n, axis=axis, clip_mask=clip_mask, return_clip_mask=return_clip_mask
        )
    else:
        res = sigma_clipped_axis_nanmean(
            x, n=n, axis=axis, clip_mask=clip_mask, return_clip_mask=return_clip_mask
        )
    return res


def sigma_clipped_axis_nanmean_fast(
    x, n=2.0, axis=0, clip_mask=None, return_clip_mask=False
):
    """
...
def sigma_clipped_axis_nanmean(
    x, n=2.0, axis=0, clip_mask=None, return_clip_mask=False
):
    """
    ...
    """
    if isNone(clip_mask):
        sig_mad, median = nan_median_absolute_deviation(
            x, axis=axis, scaled=True, return_median=True
        )
...
def sigma_clipped_axis_nanmean_gpu(
    x, n=2.0, axis=0, clip_mask=None, return_clip_mask=False
):
    """
    Computes nanmean of x along the indicated axis, clipping values further than n MAD from the median (non-iteratively).
    """
    if isNone(clip_mask):
        sig_mad, median = nan_median_absolute_deviation_gpu(
            x, axis=axis, scaled=True, return_median=True
        )
...
def ang_size_to_proj_sep(ang_size, distance=None):
    ...
    if isNone(distance):
        if "d" in target_defaults:
            ...
            raise ValueError(
                "Argument 'distance' must either be specified or set in the target_defaults"
                "dictionary (under key 'd')"
            )
    ang_size = (
        ang_size << u.arcsec
    )  # If units aren't provided, sets unit to arcsec. Else converts unit to arcsec
    d = distance << u.pc
...
def crop_data(data, new_shape, cent=None):
    ...
    x0, y0 = cent
    x1, y1 = (
        max(0, int(np.round(x0 - (new_nx - 1.0) / 2.0))),
        max(0, int(np.round(y0 - (new_ny - 1.0) / 2.0))),
    )
...
def pad_or_crop_image(
    ...
    im_out = propagate_nans_in_spatial_operation(
        ...
        fn_kwargs=dict(
            new_cent=new_cent, cval=cval, order=order, mode=mode, prefilter=prefilter
        ),
        fn_nan_kwargs=dict(
            new_cent=new_cent, cval=cval, order=order, mode=mode, prefilter=False
        ),
        prop_threshold=prop_threshold,
    )
    return im_out
...
def pad_or_crop_about_pos(
    ...
    nd = np.ndim(im)
    xg, yg = np.meshgrid(
        np.arange(nx_new, dtype=np.float64), np.arange(ny_new, dtype=np.float64)
    )
...
def median_filter_sequence(
    im, radius=None, size=None, footprint=None, prop_threshold=1e-6
):
    im = np.asarray(im)
    nd = np.ndim(im)
    fn_args = []
    ...
    if not isNone(radius) and isNone(footprint):
        ...
        rc0 = np.array([(rnx - 1) / 2.0, (rnx - 1) / 2.0])
    ...
    if nd == 2:
        ...
        im_out = np.zeros((nI, ny, nx), dtype=im.dtype)
        for i in range(nI):
            im_out[i] = propagate_nans_in_spatial_operation(
                im_reshaped[i],
                ndimage.median_filter,
                fn_args=fn_args,
                fn_kwargs=fn_kwargs,
...
def convolve_with_fwhm_tophat(im, fwhm, prop_threshold=1e-6):
    ...
    psf_kernel = Tophat2DKernel(fwhm / 2.0, mode="oversample", factor=100).array
    psf_kernel /= np.nanmax(psf_kernel)
    fn_args, fn_kwargs = [psf_kernel], dict(mode="same")
    if nd == 2:
        im_con = propagate_nans_in_spatial_operation(
            im,
            signal.fftconvolve,
            fn_args=fn_args,
            fn_kwargs=fn_kwargs,
        ...
        for i in range(nI):
            im_con[i] = propagate_nans_in_spatial_operation(
                im_reshaped[i],
                signal.fftconvolve,
                fn_args=fn_args,
                fn_kwargs=fn_kwargs,
...
def model_Fpol(
    ...
    max_radius_in_fov = ang_size_to_proj_sep(
        px_size_to_ang_size(np.max(rmap) / np.cos(np.deg2rad(incl)), pixscale), distance
    ).value
    ...
    s_im = np.pi - np.arccos(
        np.sin(ang_tmp) * np.cos(np.pi + pg) * np.sin(incl_rad)
        + np.cos(ang_tmp) * np.cos(incl_rad)
    )

    image_xy = np.array([x_im, y_im]).T

    xgrid, ygrid = np.meshgrid(
        (np.arange(nx, dtype=np.float32) - cent[0]) * pixscale * distance,
        (np.arange(ny, dtype=np.float32) - cent[1]) * pixscale * distance,
    )
...
def model_rproj(
    ...
    grid = np.array(
        [
            i.flatten() * pixscale * distance
            for i in np.meshgrid(
                np.arange(nx, dtype=np.float32) - cent[0],
                np.arange(ny, dtype=np.float32) - cent[1],
            )
        ]
    ).T
...
def imcube_to_adi_hcube(
    imcube, parangs, ncores=-2, use_gpu=False, cent=None, cval0=0.0
):
    hcube = rotate_hypercube(
        np.tile(imcube[np.newaxis], [len(parangs), 1, 1, 1]),
        parangs,
        cent=cent,
        cval0=cval0,
        ncores=ncores,
...
def build_sequence_hcube(
    filepaths,
    parang_key="parang",
    ncores=-2,
    return_headers=False,
    print_approx_ram_usage=False,
    only_hcube=False,
    memmap=False,
    ext=1,
):
    ...
    imlist, headers = zip(
        *Parallel(n_jobs=ncores, prefer="threads")(
            delayed(fits.getdata)(f, header=True, memmap=memmap, ext=ext)
            for f in filepaths
        )
    )
    ...
    if print_approx_ram_usage:
        ...
        print(
            "Storing the image sequence in RAM requires ~{0:0.2f} GB".format(
                sys.getsizeof(hcube) / 1e9
            )
        )
    if only_hcube:
        return hcube
...
try:
    ...
    print(
        "CuPy succesfully imported. Using GPU where applicable. "
