...
cc_peak_values = CSV.read("/Users/mileslucas/research/papers/HD169142_disk_paper/src/data/location_cross_correlation_peaks.csv", DataFrame, header=false)
...
inner_peak_motion = (cc_peak_values[1, :Column2] ± cc_peak_values[1, :Column3])u"°/yr"
...
zscore_21 = ttest(kep_21, inner_peak_motion)
push!(lines, "zscore for inner ring peaks vs Kep: $zscore_21")
zscore_37 = ttest(kep_37, inner_peak_motion)
push!(lines, "zscore for inner ring peaks vs 37au: $zscore_37")
zscore_0 = abs(Measurements.value(inner_peak_motion) / Measurements.uncertainty(inner_peak_motion))
push!(lines, "zscore for inner ring peaks vs none: $zscore_0")
