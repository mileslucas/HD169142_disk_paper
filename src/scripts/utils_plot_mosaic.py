...
### Mosaic plots
def plot_mosaic(stokes_cube, iwa, idx=4):
    # prepare some things we need
    Qphi_frames = stokes_cube[:, idx]
...
def plot_rdi_mosaic(rdi_cube, iwa, smooth=True):
    # prepare some things we need
    rdi_frames = rdi_cube
    Qphi_sum = np.nansum(rdi_frames, axis=0)
    rs = frame_radii(rdi_cube) * plate_scale / 1e-3

    side_length = rdi_cube.shape[-1] * plate_scale * 1e-3 / 2
    ext = (side_length, -side_length, -side_length, side_length)
    ...
    for rdi_frame, ax, title in zip(rdi_frames, axes, titles):
