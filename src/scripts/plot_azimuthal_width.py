...
fig, axes = pro.subplots(
    nrows=4, ncols=2, width="7in", height="3in", wspace=0.5, hspace=0, sharey=4, spanx=False
)
...
def moving_average(arr, err, window_size):
    ...
    vals = np.convolve(arr_pad, norm_window, "valid")[window_size // 2 : -window_size // 2 - 1]
    vars = np.convolve(var_pad, norm_window, "valid")[window_size // 2 : -window_size // 2 - 1]
    return vals, np.sqrt(vars / window_size)


for i, date in enumerate(tqdm.tqdm(dates)):
    # load data
    table = pd.read_csv(paths.data / date / f"{date}_HD169142_vampires_azimuthal_profiles.csv")
    subtable = table.query("region == 'inner'")
    groups = subtable.groupby("filter")

    # load data
    with fits.open(paths.data / date / f"{date}_HD169142_vampires_Qphi_polar.fits") as hdul:
    ...
    for wl_idx in range(polar_cut.shape[0]):
        ...
        ax2.plot(azimuth, values, lw=1, c="0.5", zorder=0)
