...
for i, date in enumerate(tqdm.tqdm(dates)):
    ...
    for wl_idx in range(polar_cut.shape[0]):
        ...
        for az_idx in range(polar_cut.shape[-1]):
            ...
            peak_rads.append(res.mean.value + min_idx)
            cov_matrix = fitter.fit_info['param_cov']
            uncertainties = np.sqrt(np.diag(cov_matrix))
            peak_errs.append(uncertainties[1])
        ...
        axes[wl_idx, i].plot(
            ...
            np.array(peak_rads) * plate_scale * dist,
            shadedata=np.array(peak_errs) * plate_scale * dist,
            c = cycle[wl_idx],
            zorder=10
        )
...
axes.format(
    xlabel="Angle E of N (°)",
    ylabel="Radius of peak value (au)",
...
fig.savefig(paths.figures / "HD169142_vampires_azimuthal_peaks.pdf", bbox_inches="tight", dpi=300)
