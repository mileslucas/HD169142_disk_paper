...
if __name__ == "__main__":
    ...
    fig, axes = pro.subplots(nrows=8, width=f"{width}in", refheight=f"{refheight}in", hspace=0.5)
    ...
    for i, folder in enumerate(tqdm.tqdm(folders)):
        ...
        # PDI images
        image = keplerian_warp(
            polar_cube[mask, :],
            rs[mask] * target_info.dist_pc * pxscales[folder],
            timestamps[i],
            t0,
        )
    ...
    fig.savefig(paths.figures / "HD169142_polar_Qphi_inner_warped_subbed.pdf", bbox_inches="tight")
