...
if __name__ == "__main__":
    ...
    for i, folder in enumerate(tqdm.tqdm(folders)):
        ...
        im = axes[i, 0].imshow(
            polar_cube_masked, extent=ext, norm=norm, vmin=norm.vmin, vmax=norm.vmax
        )
        # axes[0].colorbar(im)
        ...
        im = axes[i, 1].imshow(
            polar_cube_warped, extent=ext, norm=norm, vmin=norm.vmin, vmax=norm.vmax
        )
        # axes[0].colorbar(im)
        labels = label_from_folder(folder).split()
        axes[i, 0].text(
    ...
    for ax in axes:
        for offset in (90, 270):
            ax.vlines(
                offset + target_info.pos_angle, ax.get_ylim()[0], 30, c="0.9", lw=0.5, ls="--"
            )
    ...
    ## sup title
    axes.format(aspect="auto", xlabel="Angle E of N (°)", ylabel="", xlocator=90)
    axes[:, 0].format(xlocator=[0, 90, 180, 270])

    fig.savefig(paths.figures / "HD169142_polar_Qphi_inner.pdf", bbox_inches="tight")
