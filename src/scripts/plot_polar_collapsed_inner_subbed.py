...
if __name__ == "__main__":
    ...
    # load data
        ...
        # norm = simple_norm(norm_image, stretch="sinh", sinh_a=0.5)
        norm = pro.DivergingNorm()
        im = axes[i].imshow(norm_image, extent=ext, cmap="div", norm=norm)
        # axes[0].colorbar(im)
        labels = label_from_folder(folder).split()
        axes[i].text(
            0.03, 0.95, labels[0], transform="axes", c="0.3 ", ha="left", va="top", fontsize=8, fontweight="bold"
        )
        axes[i].text(
            0.99, 0.95, labels[1], transform="axes", c="0.3 ", ha="right", va="top", fontsize=8, fontweight="bold"
        )
