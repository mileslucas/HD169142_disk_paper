...
from instrument_info import gpi_info, irdis_info, zimpol_info
...
def process_vampires_data(folder, filename):
    vampires_stokes_cube, vampires_hdr = fits.getdata(filename, header=True)
    crop_size = 400

    vampires_Qphi = crop(vampires_stokes_cube[4], crop_size)
    get_diskmap_outputs(
        vampires_Qphi,
        name=paths.data / folder / "diskmap" / f"{folder}_HD169142_diskmap_Qphi",
        pxscale=vampires_hdr["PXSCALE"] / 1e3,
        rmax=200,
    )
    vampires_Uphi = crop(vampires_stokes_cube[5], crop_size)
    get_diskmap_outputs(
        vampires_Uphi,
        name=paths.data / folder / "diskmap" / f"{folder}_HD169142_diskmap_Uphi",
        pxscale=vampires_hdr["PXSCALE"] / 1e3,
        rmax=200,
    )
...
def process_zimpol_data(folder: str):
    zpl_Qphi, _ = fits.getdata(filename=paths.data / folder / "Qphi.fits", header=True)
    ...
    zpl_Uphi, _ = fits.getdata(filename=paths.data / folder / "Uphi.fits", header=True)
...
def process_irdis_data(folder: str, filename: Path):
    cube, _ = fits.getdata(filename, header=True)
...
def process_naco_data(folder: str):
    ...
    naco_Uphi, naco_Uphi_hdr = fits.getdata(
        filename=paths.data / folder / "coadded" / "U_phi.fits",
        header=True,
        ext=("U_PHI_CTC_IPS", 1),
    )
...
def process_charis_data(folder: str):
    cube, hdr = fits.getdata(
        filename=paths.data / folder / f"{folder}_HD169142_stokes_collapsed.fits", header=True
    )
